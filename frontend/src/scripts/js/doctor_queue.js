...
function renderUsers(users) {
    ...
    users.forEach(function (user, index) {
        ...
        var row = "<tr class=\"".concat(rowClass, "\">\n            <td>").concat(user.patient.first_name, " ").concat(user.patient.last_name, "</td>\n            <td>").concat(user.patient.patient_id, "</td>\n            <td>").concat(user.patient.date_of_birth, "</td>\n            <td>").concat(user.status === 2
            ...
            : user.status === 3
                ? "Resolved Pending"
                : "Not Pending", "</td>\n            <td>\n                <button class=\"btn btn-primary btn-sm\" onclick=\"viewRecordHistory(").concat(index, ")\">View Record</button>\n                <button class=\"btn btn-secondary btn-sm pend-btn\" style=\"background-color: yellow;\" onclick=\"pendUser(").concat(index, ")\">\n                    ").concat(user.status === 2
            ? "Unpend"
            : user.status === 3
                ? "Pend"
                : "Pend", "\n                </button>\n                <button class=\"btn btn-success btn-sm\" style=\"background-color: green;\" onclick=\"completeUser(").concat(index, ")\">Complete</button>\n            </td>\n        </tr>");
